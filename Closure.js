...
function after(count, func) {
    let countForExecute = 1; 
    return (() => {
        if(count <= countForExecute) func()
        countForExecute++
    })
}
...
function rollCall(names) {
    let index = 0;
    return (() => console.log(names[index++] ?? "Everyone accounted for"))
}
...
function makeHistory(limit) {
    ...
    return ((task) => {
        ...
        if(task == "undo") {
            if(taskStack.length != 0) result = `${taskStack.pop()} undone`;
        } else {
            taskStack.push(task);
            result = `${task} done`;
        }
